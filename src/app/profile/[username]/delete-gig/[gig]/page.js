...
export default async function DeleteGigPage({params}){
  ...
  async function handleSubmit(){
    ...
    return(<>
      ...
      <p className={styles.p}>*** <i>This function is only available when no users have an active ~going~ status for this gig </i>***</p>
      <p className={styles.p}>{`${data.title} - ${data.location} - ${dateString}`}</p>
